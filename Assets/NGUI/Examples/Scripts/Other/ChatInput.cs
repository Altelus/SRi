...
public class ChatInput : Photon.MonoBehaviour
{
	...
	void OnSubmit ()
	{
		if (textList != null)
		{
            ...
            photonView.RPC("AddChatMessage", PhotonTargets.Others, text);

			if (!string.IsNullOrEmpty(text))
			{
				textList.Add("[FFFFFF]Me: "+text);
				mInput.text = "";
    ...
    [RPC]
    void AddChatMessage(string msg, PhotonMessageInfo mi)
    {
        if (!string.IsNullOrEmpty(msg))
        {
            textList.Add("[AAAAAA]" + mi.sender.name + ": " + msg);
        }
    }
}